...
import project.moneymanager_api.dto.IncomeDTO;
import project.moneymanager_api.entity.CategoryEntity;
import project.moneymanager_api.entity.IncomeEntity;
import project.moneymanager_api.entity.ProfileEntity;
import project.moneymanager_api.repository.CategoryRepository;
import project.moneymanager_api.repository.IncomeRepository;
import project.moneymanager_api.service.mapper.IncomeMapper;
...
public class IncomeService {
    ...
    public List<IncomeDTO> filterIncomes(LocalDate startDate, LocalDate endDate, String keyword, Sort sort){
        ProfileEntity profile = profileService.getCurrentProfile();
        return incomeRepository.findByProfileIdAndDateBetweenAndNameContainingIgnoreCase(profile.getId(), startDate, endDate, keyword, sort).stream().map(IncomeMapper.getInstance()::toDto).toList();
    }
}